...
void Program::Run(int8_t calcSpeed, int16_t heading, int8_t calcTurn) {
	...
	float M1 = std::round(rawSpeed * std::sin(θ1 * RAD_CONST)); //using round so there is limited drift due to pi being an approximation
	M1 += calcTurn * SPEED_CONST; //Add rotation, input is 0 - 100

	int16_t θ2 = FRONT_RIGHT_WHEEL - heading;
	float M2 = std::round(rawSpeed * std::sin(θ2 * RAD_CONST));
	M2 += calcTurn * SPEED_CONST;

	int16_t θ3 = FRONT_LEFT_WHEEL - heading;
	float M3 = std::round(rawSpeed * std::sin(θ3 * RAD_CONST));
	M3 += calcTurn * SPEED_CONST;

	std::cout << "Back wheel speed: " << M1 << '\n' << "Right Wheel Speed: " << M2 << std::endl;
}
